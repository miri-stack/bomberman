...
def game_events_occurred(self, old_game_state: dict, self_action: str, new_game_state: dict, events: List[str]):
    ...
    old_state = state_to_features(old_game_state_adjusted, distance = hp.WINDOW_LENGTH).reshape(1,self.input_dim)

    # Get new_game_state
    new_game_state_adjusted = rotate_map(new_game_state, self.orientation)
    new_state = state_to_features(new_game_state_adjusted, distance = hp.WINDOW_LENGTH).reshape(1,self.input_dim)
...
def end_of_round(self, last_game_state: dict, last_action: str, events: List[str]):
    ...
    self.transitions.append(Transition(state_to_features(last_game_state, distance = hp.WINDOW_LENGTH).reshape(1,self.input_dim), last_action, None, reward_from_events(self, events)))
